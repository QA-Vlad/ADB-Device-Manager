...
@State(
    name = "AdbRandomizerLoggingConfiguration",
    storages = [Storage("adb-randomizer-logging.xml")]
)
@Service
class LoggingConfiguration : PersistentStateComponent<LoggingConfiguration.State> {
    ...
    init {
        ...
        val logLevel = System.getProperty("adb.randomizer.log.level")
        val logCategory = System.getProperty("adb.randomizer.log.category")
            ...
            try {
                ...
                if (logCategory != null) {
                    ...
                    if (category != null) {
                        ...
                        println("ADB_Randomizer: Set log level for $category to $level")
                    }
                } else {
                    // Устанавливаем глобальный уровень
                    setGlobalLogLevel(level)
                    println("ADB_Randomizer: Set global log level to $level")
                }
            } catch (_: IllegalArgumentException) {
                println("ADB_Randomizer: Invalid log level: $logLevel")
            }
        }
    }
