...
class AdbConfigurationDialog(
    ...
    private val adbPathField = JBTextField().apply {
        ...
        println("ADB_Device_Manager [Dialog]: Initial saved path: '$savedPath'")
        
        text = if (savedPath.isNotBlank()) {
            val file = File(savedPath)
            // Проверяем что файл или директория существует
            if (file.exists()) {
                println("ADB_Device_Manager [Dialog]: Saved path exists, using: '$savedPath'")
                savedPath
            } else {
                println("ADB_Device_Manager [Dialog]: Saved path doesn't exist, searching for ADB...")
                // Если старый путь не существует, пытаемся найти ADB автоматически
                val foundPath = AdbPathResolver.findAdbExecutable() ?: ""
                println("ADB_Device_Manager [Dialog]: Found ADB at: '${foundPath.ifEmpty { "<not found>" }}'")
                foundPath
            }
        } else {
            println("ADB_Device_Manager [Dialog]: No saved path, searching for ADB...")
    ...
    private fun setupValidationListener() {
        adbPathField.document.addDocumentListener(object : DocumentListener {
            ...
            fun validateAdbPath() {
                ...
                println("ADB_Device_Manager [Dialog]: Validating path: '$path'")
                ...
                if (isWindows) {
                    path = path.replace('/', '\\')
                    println("ADB_Device_Manager [Dialog]: Normalized Windows path: '$path'")
                }
                
                val file = File(path)
                println("ADB_Device_Manager [Dialog]: File exists: ${file.exists()}, Is file: ${file.isFile}, Is directory: ${file.isDirectory}")
    ...
    private fun applyAdbPath() {
        var path = adbPathField.text.trim()
        println("ADB_Device_Manager [Dialog]: Apply button clicked with path: '$path'")
        
        // Нормализуем путь для Windows
        val isWindows = System.getProperty("os.name").startsWith("Windows")
        if (isWindows && path.isNotBlank()) {
            path = path.replace('/', '\\')
            println("ADB_Device_Manager [Dialog]: Normalized path for saving: '$path'")
        }
        ...
        if (path.isBlank()) {
            ...
            println("ADB_Device_Manager [Dialog]: Saved empty path for auto-detect")
        ...
        val file = File(path)
        println("ADB_Device_Manager [Dialog]: Checking file for save - exists: ${file.exists()}, isFile: ${file.isFile}, isDirectory: ${file.isDirectory}")
        
        // Валидируем путь
        val pathToSave = when {
            file.isFile && isValidAdbPath(path) -> {
                println("ADB_Device_Manager [Dialog]: Path is a valid ADB file")
            ...
            file.isDirectory -> {
                ...
                println("ADB_Device_Manager [Dialog]: Path is directory, checking for $adbName inside")
                println("ADB_Device_Manager [Dialog]: ADB file path: ${adbFile.absolutePath}, exists: ${adbFile.exists()}")
                
                if (adbFile.exists() && isValidAdbPath(adbFile.absolutePath)) {
                    println("ADB_Device_Manager [Dialog]: Directory contains valid ADB")
                    path
                } else {
                    println("ADB_Device_Manager [Dialog]: Directory does not contain valid ADB")
                ...
                println("ADB_Device_Manager [Dialog]: Path does not exist or is invalid")
                JOptionPane.showMessageDialog(
                    this.contentPane,
        ...
        println("ADB_Device_Manager [Dialog]: Saving path to settings: '$pathToSave'")
        // Сохраняем путь
        PluginSettings.instance.adbPath = pathToSave
        PluginLogger.debug(LogCategory.GENERAL, "ADB path saved: %s", pathToSave)
        println("ADB_Device_Manager [Dialog]: Path saved successfully")
    ...
    private fun validateAdbExecutable(path: String): AdbValidationResult {
        println("ADB_Device_Manager [Dialog]: validateAdbExecutable called with path: '$path'")
        try {
            val file = File(path)
            println("ADB_Device_Manager [Dialog]: File check - exists: ${file.exists()}, isFile: ${file.isFile}, absolutePath: ${file.absolutePath}")
            
            if (!file.exists() || !file.isFile) {
                println("ADB_Device_Manager [Dialog]: File not found or not a file")
            ...
            if (isWindows && file.parentFile != null) {
                ...
                println("ADB_Device_Manager [Dialog]: Checking DLLs:")
                println("ADB_Device_Manager [Dialog]:   AdbWinApi.dll exists: ${adbWinApi.exists()}")
                println("ADB_Device_Manager [Dialog]:   AdbWinUsbApi.dll exists: ${adbWinUsbApi.exists()}")
    ...
    private fun isValidAdbPath(path: String): Boolean {
        println("ADB_Device_Manager [Dialog]: isValidAdbPath called with: '$path'")
        try {
            val file = File(path)
            println("ADB_Device_Manager [Dialog]: isValidAdbPath - file exists: ${file.exists()}, isFile: ${file.isFile}")
            
            if (!file.exists() || !file.isFile) {
                PluginLogger.debug(LogCategory.GENERAL, "ADB validation failed - file does not exist or is not a file: %s", path)
                println("ADB_Device_Manager [Dialog]: isValidAdbPath returning false - file not found")
                return false
            }
            ...
            if (!isWindows && !file.canExecute()) {
                ...
                println("ADB_Device_Manager [Dialog]: isValidAdbPath returning false - not executable")
                return false
            }
            ...
            if (!file.name.equals(adbName, ignoreCase = true)) {
                ...
                println("ADB_Device_Manager [Dialog]: isValidAdbPath returning false - wrong filename: ${file.name}, expected: $adbName")
                return false
            }
            
            println("ADB_Device_Manager [Dialog]: isValidAdbPath - trying to run ADB version")
            ...
            if (file.parentFile != null) {
                processBuilder.directory(file.parentFile)
                println("ADB_Device_Manager [Dialog]: isValidAdbPath - process working directory: ${file.parentFile.absolutePath}")
            }
            
            println("ADB_Device_Manager [Dialog]: isValidAdbPath - starting process...")
            val process = processBuilder.start()
            val finished = process.waitFor(5, TimeUnit.SECONDS)
            
            if (!finished) {
                PluginLogger.debug(LogCategory.GENERAL, "ADB validation failed - timeout waiting for version command")
                println("ADB_Device_Manager [Dialog]: isValidAdbPath - timeout waiting for ADB version")
                process.destroyForcibly()
                return false
            }
            ...
            if (exitCode != 0) {
                ...
                println("ADB_Device_Manager [Dialog]: isValidAdbPath - ADB returned exit code: $exitCode, error: $error")
                return false
            }
            
            val output = process.inputStream.bufferedReader().readText()
            println("ADB_Device_Manager [Dialog]: isValidAdbPath - ADB version output: ${output.take(100)}")
            val isValid = output.contains("Android Debug Bridge", ignoreCase = true)
            ...
            if (!isValid) {
                ...
                println("ADB_Device_Manager [Dialog]: isValidAdbPath returning false - output doesn't contain 'Android Debug Bridge'")
            } else {
                PluginLogger.debug(LogCategory.GENERAL, "ADB validation successful for path: %s", path)
                println("ADB_Device_Manager [Dialog]: isValidAdbPath returning true - ADB is valid!")
            }
            
            return isValid
        } catch (e: Exception) {
            PluginLogger.debug(LogCategory.GENERAL, "Failed to validate ADB path '%s': %s", path, e.message ?: "Unknown error")
            println("ADB_Device_Manager [Dialog]: isValidAdbPath exception: ${e.message}")
            e.printStackTrace()
            return false
        }
    }
